...
- **Layered maximum speeds** — separate upload and download ceilings for inbounds, clients, and individual client–inbound links; the client-wide ceiling always applies. WireGuard enforces client rules per peer. TUIC and MTProto sidecars support aggregate inbound ceilings; their individual secrets do not yet have separate speed enforcement.
- **Traffic and expiry policies** — fixed or rolling window quotas, stop or slow continuation after quota exhaustion, configurable overage debit factors, and low-speed grace for a chosen number of days after expiry. Inbound traffic debit factors start at 0.01× and can be freely increased within safe integer precision; TUIC remains at 1× because its encrypted sidecar cannot attribute bytes to authenticated clients. Client views, share pages, and subscription usage show uploaded and downloaded quota usage after the factor; raw transfer bytes remain available internally for reconciliation, while inbound server capacity is measured in actual bytes. The share page displays each node's factor, remaining window quota, fixed-window reset countdown, and maximum speed without exposing internal policy actions.
