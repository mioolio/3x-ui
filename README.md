...
- **Traffic and expiry policies** — fixed or rolling window quotas, stop or slow continuation after quota exhaustion, configurable overage debit factors, and low-speed grace for a chosen number of days after expiry. Inbound traffic debit factors start at 0.01× and can be freely increased within safe integer precision; TUIC remains at 1× because its encrypted sidecar cannot attribute bytes to authenticated clients. The share page displays each node's factor, remaining window quota, fixed-window reset countdown, and maximum speed without exposing internal policy actions.
