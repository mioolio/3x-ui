...
- **分层最高速度** — 分别设置入站、客户端、关联入站的上行和下行最高速度；客户端总上限始终有效。WireGuard 支持按 peer 执行客户端策略；TUIC 和 MTProto 侧车支持入站聚合最高速度，侧车协议的单客户端密钥速度尚不可单独执行。
- **流量与到期策略** — 固定或滚动窗口配额、额度用完后停止或低速继续、可配置的超额扣量倍率，以及到期后按天设置的低速宽限。入站流量扣量倍率从 0.01× 起向上自由设置（受安全整数精度约束）；TUIC 因加密侧车无法按客户端归因流量，保持 1×。客户端面板、分享页和订阅用量按倍率展示已上传、已下载及剩余额度；底层保留实际传输字节用于核对，入站服务器容量仍按实际传输计算。分享页展示节点倍率、窗口剩余、下次固定窗口重置倒计时与最高速度，不公开内部处理策略。
