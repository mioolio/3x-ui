...
- **流量与到期策略** — 固定或滚动窗口配额、额度用完后停止或低速继续、可配置的超额扣量倍率，以及到期后按天设置的低速宽限。入站流量扣量倍率从 0.01× 起向上自由设置（受安全整数精度约束）；TUIC 因加密侧车无法按客户端归因流量，保持 1×。分享页展示节点倍率、窗口剩余、下次固定窗口重置倒计时与最高速度，不公开内部处理策略。
