...
export const InboundDbFieldsSchema = z.object({
  ...
  planPeriod: z.enum(['', 'daily', 'weekly', 'monthly']).default(''),
  planQuotaGB: z.number().int().min(0).default(0),
  planAction: z.enum(['', 'disable', 'throttle']).default(''),
  planSpeed: z.number().int().min(0).default(0),
