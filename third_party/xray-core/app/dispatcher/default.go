...
func (d *DefaultDispatcher) getLink(ctx context.Context) (*transport.Link, *transport.Link) {
	...
	if user != nil && len(user.Email) > 0 {
		...
		if sessionInbound.Tag != "" {
			if c, _ := d.stats.GetOrRegisterCounter(panelTrafficName(sessionInbound.Tag, user.Email, "uplink")); c != nil {
				inboundLink.Writer = &SizeStatWriter{Counter: c, Writer: inboundLink.Writer}
			}
			if c, _ := d.stats.GetOrRegisterCounter(panelTrafficName(sessionInbound.Tag, user.Email, "downlink")); c != nil {
				outboundLink.Writer = &SizeStatWriter{Counter: c, Writer: outboundLink.Writer}
			}
		}
		inboundLink.Writer, outboundLink.Writer = panelWrapWriters(ctx, d.stats, sessionInbound.Tag, user.Email, inboundLink.Writer, outboundLink.Writer)
	}
...
func WrapLink(ctx context.Context, policyManager policy.Manager, statsManager stats.Manager, link *transport.Link) *transport.Link {
	...
	if user != nil && len(user.Email) > 0 {
		...
		var inboundCounter stats.Counter
		if sessionInbound.Tag != "" {
			inboundCounter, _ = statsManager.GetOrRegisterCounter(panelTrafficName(sessionInbound.Tag, user.Email, "uplink"))
			if c, _ := statsManager.GetOrRegisterCounter(panelTrafficName(sessionInbound.Tag, user.Email, "downlink")); c != nil {
				link.Writer = &SizeStatWriter{Counter: c, Writer: link.Writer}
			}
		}
		link.Reader = panelWrapReader(ctx, statsManager, sessionInbound.Tag, user.Email, link.Reader.(*buf.TimeoutWrapperReader), inboundCounter)
		_, link.Writer = panelWrapWriters(ctx, statsManager, sessionInbound.Tag, user.Email, nil, link.Writer)
	}
