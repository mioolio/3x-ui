...
func (s *Server) GetUserByAddr(ctx context.Context, addr netip.Addr) (user *protocol.MemoryUser) {
	...
	ambiguous := false
	s.users.Range(func(key, value any) bool {
		candidate := value.(*protocol.MemoryUser)
		peer := candidate.Account.(*MemoryAccount)
		for i := range peer.AllowedIPs {
			prefix := peer.AllowedIPs[i]
			...
			if prefix.Bits() > bestBits {
				...
				ambiguous = false
			} else if prefix.Bits() == bestBits && user != candidate {
				// Duplicate exact prefixes can survive in older databases. Never
				// attribute one peer's usage to an arbitrary other peer.
				ambiguous = true
			}
		}
		return true
